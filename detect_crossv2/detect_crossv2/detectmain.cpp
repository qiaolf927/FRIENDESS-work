...
int main( int argc, char** argv )
{
	...
	double crosslinewidth = 30.0;//十字线线宽
	...
	//闭运算
	morphologyEx(imedge,imedge,MORPH_CLOSE,getStructuringElement(MORPH_ELLIPSE,Size(crosslinewidth,crosslinewidth)));
	...
	for (int i = 0;i<contoursfilt.size();i++)
	{
		...
		//6.找平行线，以两种精度构建，一组平行线存入thetacluster中，参数为clusterthetavalue与clusterrouvalue
		//没rough是较高精度聚类，theta差3°以内；有rough是粗精度聚类，theta差10°以内
		for (int k = 0;k<lineparameter.size();k++)
		{
			rou = lineparameter[k][0];
			theta = lineparameter[k][1];
